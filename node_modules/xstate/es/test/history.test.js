...
describe('deep history states', function () {
    ...
    describe('history', function () {
        ...
        it('should go to the deep history (explicit)', function () {
            // on.second.B.P -> off
            var stateOff = historyMachine.transition(state2BP, 'POWER');
            assert.deepEqual(historyMachine.transition(stateOff, 'DEEP_POWER').value, { on: { second: { B: 'P' } } });
        });
        it('should go to the deepest history', function () {
            // on.second.B.Q -> off
            var stateOff = historyMachine.transition(state2BQ, 'POWER');
            assert.deepEqual(historyMachine.transition(stateOff, 'DEEP_POWER').value, { on: { second: { B: 'Q' } } });
        });
    });
});
describe('parallel history states', function () {
        ...
        states: {
            off: {
                on: {
                    ...
                    PARALLEL_DEEP_HISTORY: [{ target: ['on.A.deepHistory', 'on.K.deepHistory'] }]
                }
            },
            on: {
                type: 'parallel',
    ...
    describe('history', function () {
        ...
        it('should remember first level state history', function () {
            var stateOff = historyMachine.transition(stateACDKL, 'POWER');
            assert.deepEqual(historyMachine.transition(stateOff, 'DEEP_POWER').value, {
                on: { A: { C: 'D' }, K: 'L' }
            });
        });
        ...
        it('should re-enter multiple history states', function () {
            var stateOff = historyMachine.transition(stateACEKMO, 'POWER');
            assert.deepEqual(historyMachine.transition(stateOff, 'PARALLEL_HISTORY').value, {
                on: { A: { C: 'D' }, K: { M: 'N' } }
            });
        });
        it('should re-enter a parallel with partial history', function () {
            var stateOff = historyMachine.transition(stateACEKMO, 'POWER');
            assert.deepEqual(historyMachine.transition(stateOff, 'PARALLEL_SOME_HISTORY').value, {
                on: { A: { C: 'D' }, K: { M: 'N' } }
            });
        });
        it('should re-enter a parallel with full history', function () {
            var stateOff = historyMachine.transition(stateACEKMO, 'POWER');
            assert.deepEqual(historyMachine.transition(stateOff, 'PARALLEL_DEEP_HISTORY').value, {
                on: { A: { C: 'E' }, K: { M: 'O' } }
            });
        });
    });
...
describe('multistage history states', function () {
    var pcWithTurboButtonMachine = Machine({
    ...
    it('should go to the most recently visited state', function () {
        ...
        assert.deepEqual(pcWithTurboButtonMachine.transition(loadingState, 'STARTED').value, { running: 'turbo' });
    });
});
